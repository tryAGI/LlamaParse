...
- `src/libs/LlamaParse/generate.sh` — Downloads spec, fixes security/servers/enum values, runs autosdk
...
- `generate.sh` injects top-level `security` array (`HTTPBearer`) and `servers` section via `jq`
